...
const CONTROLS = ['up', 'down', 'left', 'right', 'extra'];
const CONTROL_CODES = [38, 40, 37, 39, 41];
...
export function predictClass(classId) {
  if(postcard != classId){
    ...
    socket.emit('scan', classId);
  }
}

export function isPredicting() {
  // socket.emit('scan',statusElement);
...
export function loadStatus(status) {
  loadStatusElement.innerText = status;
}
...
extraButton.addEventListener('mousedown', () => handler(4));
extraButton.addEventListener('mouseup', () => mouseDown = false);
...
export function draw(image, canvas) {
  ...
  for (let i = 0; i < height * width; ++i) {
    ...
    imageData.data[j + 3] = (data[i * 3 + 3] + 1) * 127;
